...
directory_name = 'file_num_1_10_20'
...
for r, d, f in os.walk(directory):
    ...
    if f != [] and '.py' not in f[0]:
        ...
        ptol = float(tol_name[tol_name.index('ptol')+4:tol_name.index('rtol')])*100 #cm
        rtol = float(tol_name[tol_name.index('rtol')+4:])  
        ...
        for file in f:       
            if '.json' in file:
                ...
                for i, tracker in enumerate(data):
                    ...
                    if pos_error > ptol or ori_error > rtol:             
                        avg_converge += 0
                    else:
                        avg_converge += 1
                ...
                fig1, ax1 = plt.subplots(1, 6, figsize=(80, 5))    
                ...
                ax1[0].tick_params(axis='x', labelsize=small_font_size)
                ax1[0].tick_params(axis='y', labelsize=small_font_size)

                ax1[1].tick_params(axis='x', labelsize=small_font_size)
                ax1[1].tick_params(axis='y', labelsize=small_font_size)

                ax1[2].tick_params(axis='x', labelsize=small_font_size)
                ax1[2].tick_params(axis='y', labelsize=small_font_size)

                ax1[3].tick_params(axis='x', labelsize=small_font_size)
                ax1[3].tick_params(axis='y', labelsize=small_font_size)

                ax1[4].tick_params(axis='x', labelsize=small_font_size)
                ax1[4].tick_params(axis='y', labelsize=small_font_size)

                ax1[5].tick_params(axis='x', labelsize=small_font_size)
                ax1[5].tick_params(axis='y', labelsize=small_font_size)


                fig1.savefig(f'{save_act_path}/{tol_name}/{freq_name}' + '/' + file[:-5] + '.svg', bbox_inches='tight', format='svg', dpi=1200)

                x = [i/freq for i in range(len(pos_errors))]

                ax2[0].plot(x, pos_errors, 'r')

                ax2[0].set_title('Position Error', fontsize=font_size)
                ax2[0].set_ylabel('Centimeters', fontsize=font_size)
                ax2[0].set_xlabel('Time', fontsize=font_size)
                ax2[0].set_ylim((0, 10))

                ax2[1].plot(x, ori_errors, 'r')

                ax2[1].set_title('Orientation Error', fontsize=font_size)
                ax2[1].set_ylabel('Degrees', fontsize=font_size)
                ax2[1].set_xlabel('Time', fontsize=font_size)
                ax2[1].set_ylim((0, 40))
                
                # Putting tolerances on graph
                ax2[0].hlines(y=ptol, xmin=0, xmax=max(x), colors='b', linestyles='--', lw=2, label='position tolerance')
                ax2[1].hlines(y=rtol, xmin=0, xmax=max(x), colors='b', linestyles='--', lw=2, label='orientation tolerance')
                ...
                for pose in des_xs:
                    if not first:
                        # Plotting position
                        ax3D.scatter(pose[0], pose[1], pose[2], color='g', marker='*', label='reference points')
                ...
                # Putting tolerances on graph
                ax3[0][freq_to_plot_map[freq_name]].hlines(y=ptol, xmin=0, xmax=max(x), colors='b', linestyles='--', lw=2, label='position tolerance')
                ax3[1][freq_to_plot_map[freq_name]].hlines(y=rtol, xmin=0, xmax=max(x), colors='b', linestyles='--', lw=2, label='orientation tolerance')
        ...
        # This assumes 5 run were collected per trajectory
        avg_pos_error /= 5
        avg_ori_error /= 5
        avg_trajectory_time /=5
        avg_converge /= total_points 
        ...
        stat = {'freq': freq,
                'avg_convergence_percentage': avg_converge, 
        ...
        by_label = dict(zip(labels, handles))
        ax3[0][freq_to_plot_map[freq_name]].legend(by_label.values(), by_label.keys())

        handles, labels = ax3[1][freq_to_plot_map[freq_name]].get_legend_handles_labels()
        by_label = dict(zip(labels, handles))
        ax3[1][freq_to_plot_map[freq_name]].legend(by_label.values(), by_label.keys())
